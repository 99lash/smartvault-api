...
# Temporarily comment out the get_current_user() & require_admin() methods.
...
def create_user(payload: UserCreate, current_user = Depends(get_current_admin), db: Session = Depends(get_db)):
...
def get_user(user_id: int, current_user = Depends(get_current_admin), db: Session = Depends(get_db)):
    """
...
def get_users_sharing_vault_access(user_id: int, current_user = Depends(get_current_user), db: Session = Depends(get_db)):
    """
